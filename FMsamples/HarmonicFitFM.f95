...
!             Allocating these type FM arrays should not fail, since these are not
!             yet allocating the space where the actual high-precision values will
!             go.  Setting K or N too large may produce another error message later
!             as the values get defined and FM tries to allocate that space.
