      ...
      MODULE C2FM_READS
         REAL (KIND(1.0D0)), DIMENSION(:),   ALLOCATABLE :: C2FM_TEMP
         REAL (KIND(1.0D0)), DIMENSION(:,:), ALLOCATABLE :: C2FM_TEMP2
      END MODULE C2FM_READS
...
!               TRIM(ST1),TRIM(ST2)
      WRITE (*,"(/'  A minimum for function 1 is'/5X," //  &
           "' x = ',ES25.16,',   f(x) = ',ES25.16)") TO_DP(XVAL),TO_DP(FVAL)
...
!      CHARACTER(80)  :: ST1,ST2
      ...
      TYPE (FM) , EXTERNAL :: F
      TYPE (FM) :: A,B,C,D,E,EPS,XM,P,Q,R,TOL1,T2,U,V,W,FU,FV,FW,FX,X,TOL3

      SAVE :: A, B, C, D, E, EPS, XM, P, Q, R, TOL1, T2, U, V, W, FU, FV, FW, FX, X, TOL3
...
!          WRITE (KW,"('         ',3X,4X,'f(x) = ',A/)") TRIM(ST2)
          WRITE (KW,"('      J =',I3,4X,'X =',ES25.16,'    F(X) =',ES25.16)") J,TO_DP(X),  &
           TO_DP(FX)*MINV
      ENDIF
...
!          WRITE (KW,"('      J =',I3,4X,'  x  = ',A)") J,TRIM(ST1)
!          WRITE (KW,"('         ',3X,4X,'f(x) = ',A/)") TRIM(ST2)
          WRITE (KW,"('      J =',I3,4X,'X =',ES25.16,'    F(X) =',ES25.16)") J,TO_DP(U),  &
           TO_DP(FU)*MINV
      ENDIF
...
!  end of main loop
  ...
  160 XVAL = X
      ...
      IF (KPRT >= 1) THEN
          ...
          IF (MIN_OR_MAX == 1) THEN
              WRITE (KW,  &
           "('  FM_FIND_MIN.   Function ',I3,I6,' iterations.  A relative minimum"//  &
           " on interval'/13X,ES20.10,'    to ',ES20.10,'  is')")NF,J,TO_DP(AX),TO_DP(BX)
!              ST1 = FM_FORMAT('ES35.25',XVAL)
!              ST2 = FM_FORMAT('ES35.25',FVAL)
!              WRITE (KW,"(15X,'   x  = ',A)") TRIM(ST1)
!              WRITE (KW,"(15X,' f(x) = ',A)") TRIM(ST2)
              WRITE (KW,"(15X,'  x =',ES24.16,',   f(x) =',ES24.16)") TO_DP(XVAL),TO_DP(FVAL)
          ELSE
              WRITE (KW,  &
           "('  FM_FIND_MIN.   Function ',I3,I6,' iterations.  A relative maximum"//  &
           " on interval'/13X,ES20.10,'    to ',ES20.10,'  is')")NF,J,TO_DP(AX),TO_DP(BX)
!              ST1 = FM_FORMAT('ES35.25',XVAL)
!              ST2 = FM_FORMAT('ES35.25',FVAL)
!              WRITE (KW,"(15X,'   x  = ',A)") TRIM(ST1)
