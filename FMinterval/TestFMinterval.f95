...
!  This is a test program for the FM 1.3 multiple-precision interval arithmetic package.
...
!             These are the variables that are not multiple precision.
      ...
      INTEGER, SAVE :: J, K, KLOG, L, KWSAVE, NCASE, NERROR
...
!  Function that returns a interval vector result.
      ...
      IMPLICIT NONE
      TYPE (FM_INTERVAL) :: A(3),B(3),SUM1(3)
      INTEGER :: J
...
!  Function that returns a interval matrix result.
      ...
      IMPLICIT NONE
      TYPE (FM_INTERVAL) :: A(3,3),B(3,3),SUM2(3,3)
      INTEGER :: J, K
...
!  Elementary functions.
      ...
      WRITE (KLOG,*) ' '
      CORRECT = A
      MWK(START(CORRECT%LEFT)+2) = 0
      MWK(START(CORRECT%RIGHT)+2) = 0
      ERROR = ABS( LEFT_ENDPOINT(RESULT) -  LEFT_ENDPOINT(CORRECT)) +  &
              ABS(RIGHT_ENDPOINT(RESULT) - RIGHT_ENDPOINT(CORRECT))
      IF (.NOT.(ERROR < 1.0D-25)) THEN
...
!  Elementary functions.
      ...
      USE FM_INTERVAL_ARITHMETIC
      IMPLICIT NONE
      TYPE (FM_INTERVAL) :: A(3), SUMVEC1
      INTEGER :: J

      CALL FM_ENTER_USER_FUNCTION(SUMVEC1)
      ...
      CALL FM_EXIT_USER_FUNCTION(SUMVEC1)
